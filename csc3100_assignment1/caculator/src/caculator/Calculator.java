...
//可能改进：
//1.空格去除方法
//2.大量的recursion不用每次都检查括号这样，多数循环可以写到外面。尝试把函数割裂开来，避免不必要的循环
//3.大量的if判断加string的串联，可以替换为str.replace方法
...
//					if (left == 3 && i != bytes.length-1) {
//						return count("(" + String.valueOf(math3(count(p.substring(left+1, i)),"cos")) + ")" + p.substring(i+1));		
//					}
//					if (left != 3 && i == bytes.length-1) {
//						return count(p.substring(0,left-3) + "(" + String.valueOf(math3(count(p.substring(left+1, i)),"cos"))+ ")");
//					}
//					if (left == 3 && i == bytes.length-1) {
//						return count(String.valueOf(math3(count(p.substring(left+1, i)),"cos")));
//					}
//					return count(p.substring(0,left-3) + "(" + String.valueOf(math3(count(p.substring(left+1, i)),"cos")) + ")" + p.substring(i+1));
				}
...
//					if (left == 4 && i != bytes.length-1) {
//						return count("(" + String.valueOf(math3(count(p.substring(left+1, i)),"sqrt")) + ")" + p.substring(i+1));		
//					}
//					if (left != 4 && i == bytes.length-1) {
//						return count(p.substring(0,left-4) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"sqrt"))+")");
//					}
//					if (left == 4 && i == bytes.length-1) {
//						return count(String.valueOf(math3(count(p.substring(left+1, i)),"sqrt")));
//					}
//					return count(p.substring(0,left-4) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"sqrt")) + ")" + p.substring(i+1));
				}
...
//						if (left == 3 && i != bytes.length-1) {
//							return count("(" +String.valueOf(math3(count(p.substring(left+1, i)),"sin"))+ ")" + p.substring(i+1));		
//						}
//						if (left != 3 && i == bytes.length-1) {
//							return count(p.substring(0,left-3) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"sin"))+ ")");
//						}
//						if (left == 3 && i == bytes.length-1) {
//							return count(String.valueOf(math3(count(p.substring(left+1, i)),"sin")));
//						}
//						return count(p.substring(0,left-3) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"sin")) + ")" + p.substring(i+1));	
					}else {
						return count(new StringBuilder(p).replace(left-3, i+1, String.valueOf(math3(count(p.substring(left+1, i)),"tan"))).toString());
//						if (left == 3 && i != bytes.length-1) {
//							return count(String.valueOf(math3(count(p.substring(left+1, i)),"tan")) + ")" + p.substring(i+1));		
//						}
//						if (left != 3 && i == bytes.length-1) {
//							return count(p.substring(0,left-3) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"tan"))+ ")");
//						}
//						if (left == 3 && i == bytes.length-1) {
//							return count(String.valueOf(math3(count(p.substring(left+1, i)),"tan")));
//						}
//						return count(p.substring(0,left-3) +"(" +String.valueOf(math3(count(p.substring(left+1, i)),"tan")) + ")" + p.substring(i+1));
					}

				}
				return count(new StringBuilder(p).replace(left, i+1, String.valueOf(count(p.substring(left+1, i)))).toString());
//				if(left == 0 && i != bytes.length-1) {
//					return count(String.valueOf(count(p.substring(left+1, i))) + p.substring(i+1));
//				}
//				if(left != 0 && i == bytes.length-1) {
//					return count(p.substring(0,left) +String.valueOf(count(p.substring(left+1, i))));
//				}
//				if(left == 0 && i == bytes.length-1) {
//					return count(String.valueOf(count(p.substring(left+1, i))));
//				}
//				return count(p.substring(0,left) +String.valueOf(count(p.substring(left+1, i))) + p.substring(i+1));
			}
